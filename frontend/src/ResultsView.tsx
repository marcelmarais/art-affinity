...
const image_url =
  "https://d7hftxdivxxvm.cloudfront.net?height=489&quality=80&resize_to=fit&src=https%3A%2F%2Fd32dm0rphc51dk.cloudfront.net%2Fp8-dS_m_hQzxy0rFkVqJKg%2Flarger.jpg&width=445";
const Search: FC = () => {
  ...
  useEffect(() => {
    if (data) {
      setIsLoading(true);
      axios
        .post(`${process.env.REACT_APP_BACKEND_URL}/query`, {
  ...
  useEffect(() => {
    if (inputText) {
      setIsLoading(true);
      axios
        .post(`${process.env.REACT_APP_BACKEND_URL}/text-query`, {
